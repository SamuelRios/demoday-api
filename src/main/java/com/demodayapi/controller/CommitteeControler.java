...
public class CommitteeControler {
    ...
    DemodayService demodayService;
     ...
     @GetMapping("/getactivecommitee")
    public ResponseEntity <Committee> getActiveDemoday() throws IOException, MethodArgumentNotValidException {
        List<Demoday> demoday = demodayService.getDemodayInProgress();
        if (demoday == null)
            throw new TherIsNotActiveDemodayException();
            Demoday teste=demoday.get(0);
            
          Committee guarda=teste.getCommittee();
         
          System.out.println(guarda);

        return new ResponseEntity<>(guarda, HttpStatus.OK);
    }
